...
#     i = 0
#     while i < len(orders):
#         if i + 1 < len(orders):  # 如果有两个订单
#             routes.append([orders[i]['station_name'], orders[i + 1]['station_name']])  # 将两个订单分配到同一行程
#             i += 2
#         else:  # 仅剩一个订单
#             routes.append([orders[i]['station_name']])  # 仅有一个订单的行程
#             i += 1
#     return routes

# 禁忌搜索算法
# 更新路径生成函数：路径可以同时有单个和两个油站，且不局限于最后一个是单个
def create_routes(orders):
    routes = []
