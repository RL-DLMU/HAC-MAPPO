...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        if act_space.__class__.__name__ == 'Discrete':
            self.available_actions = np.ones((self.episode_length + 1, self.n_rollout_threads, num_agents, act_space.n),
                                             dtype=np.float32)
            # self.available_actions_oil = np.ones((self.episode_length + 1, self.n_rollout_threads, num_agents, 3),
            #                                  dtype=np.float32)
            # self.available_actions_warehouse = np.ones(
        ...
        # self.warehouse_type = np.zeros(
        #     (self.episode_length, self.n_rollout_threads, num_agents, act_shape), dtype=np.float32)

        self.action_log_probs = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, act_shape), dtype=np.float32)
        self.action_logits_tensor = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, 3), dtype=np.float32)
        self.oil_log_probs = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, act_shape), dtype=np.float32)
        # self.warehouse_log_probs = np.zeros(
        #     (self.episode_length, self.n_rollout_threads, num_agents, act_shape), dtype=np.float32)
        ...
        #     (self.episode_length, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
        #
        # self.action_masks = np.zeros((self.episode_length, num_agents,act_shape))
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions,action_log_probs,oil_log_probs,
               value_preds, rewards, masks,costs=None, cost_preds=None, rnn_states_cost=None,action_logits_tensor=None,bad_masks=None, active_masks=None,):
        """
        ...
        self.action_logits_tensor[self.step] = action_logits_tensor.copy()
        # self.warehouse_type[self.step]=warehouse_type.copy()

        self.action_log_probs[self.step] = action_log_probs.copy()
        self.oil_log_probs[self.step] = oil_log_probs.copy()
        # self.warehouse_log_probs[self.step] = warehouse_log_probs.copy()
        ...
        # if available_actions_oil is not None:
        #     self.available_actions_oil[self.step]=available_actions_oil.copy()
        #     self.available_actions_warehouse[self.step]=available_actions_warehouse.copy()
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        ...
        if self._use_proper_time_limits:
            ...
            if self._use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        # c=value_normalizer.denormalize(
                        #     self.value_preds[step + 1])
                        # d=value_normalizer.denormalize(self.value_preds[step])
                        delta = self.rewards[step] + self.gamma * value_normalizer.denormalize(
                            self.value_preds[step + 1]) \
                                - value_normalizer.denormalize(self.value_preds[step])
    ...
    def update_factor_oil(self, factor):
        self.factor_oil = factor.copy()
    def update_factor_warehouse(self, factor):
        self.factor_warehouse = factor.copy()
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None, cost_adv=None):
        ...
        # warehouse_type = self.warehouse_type.reshape(-1, self.warehouse_type.shape[-1])
        actions = self.actions.reshape(-1, self.actions.shape[-1])
        action_logits_tensor =self.action_logits_tensor.reshape(-1, self.action_logits_tensor.shape[-1])

        # oil_type = self.oil_type.reshape(-1, self.oil_type.shape[-1])
        ...
        oil_log_probs = self.oil_log_probs.reshape(-1, self.oil_log_probs.shape[-1])
        # warehouse_log_probs = self.warehouse_log_probs.reshape(-1, self.warehouse_log_probs.shape[-1])
        ...
        for indices in sampler:
            ...
            action_logits = action_logits_tensor[indices]
            # if self.available_actions is not None:
            ...
            old_oil_log_probs_batch = oil_log_probs[indices]
            # old_warehouse_log_probs_batch = warehouse_log_probs[indices]
            ...
            # factor_warehouse_batch = factor_warehouse[indices]
            ...
            #     adv_targ,\
            #      oil_type,old_oil_log_probs_batch
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        warehouse_type = _cast(self.warehouse_type)
        warehouse_type_log_probs = _cast(self.warehouse_log_probs)
        oil_type = _cast(self.oil_type)
        oil_type_log_probs = _cast(self.oil_log_probs)
        ...
        if self.available_actions_oil is not None:
            available_actions_oil = _cast(self.available_actions_oil[:-1])
            available_actions_warehouse = _cast(self.available_actions_warehouse[:-1])
        ...
        for indices in sampler:
            ...
            actions_batch = []
            oil_type_batch=[]
            warehouse_type_batch=[]
            available_actions_oil_batch = []
            available_actions_warehouse_batch = []
            value_preds_batch = []
            return_batch = []
            masks_batch = []
            active_masks_batch = []
            old_action_log_probs_batch = []
            old_oil_type_probs_batch = []
            old_warehouse_type_probs_batch = []
            adv_targ = []
            ...
            for index in indices:
                ...
                oil_type_batch.append(oil_type[ind:ind + data_chunk_length])
                warehouse_type_batch.append(warehouse_type[ind:ind + data_chunk_length])
                if self.available_actions_oil is not None:
                    available_actions_oil_batch.append(available_actions_oil[ind:ind + data_chunk_length])
                    available_actions_warehouse_batch.append(available_actions_warehouse[ind:ind + data_chunk_length])
                ...
                old_oil_type_probs_batch.append( oil_type_log_probs[ind:ind + data_chunk_length])
                old_warehouse_type_probs_batch.append(warehouse_type_log_probs[ind:ind + data_chunk_length])
            ...
            oil_type_batch= np.stack(oil_type_batch, axis=1)
            warehouse_type_batch = np.stack(warehouse_type_batch, axis=1)

            if self.available_actions_oil is not None:

                available_actions_oil_batch= np.stack(available_actions_oil_batch, axis=1)
                available_actions_warehouse_batch = np.stack(available_actions_warehouse_batch, axis=1)
            value_preds_batch = np.stack(value_preds_batch, axis=1)
            return_batch = np.stack(return_batch, axis=1)
            masks_batch = np.stack(masks_batch, axis=1)
            active_masks_batch = np.stack(active_masks_batch, axis=1)
            old_action_log_probs_batch = np.stack(old_action_log_probs_batch, axis=1)
            old_oil_type_probs_batch = np.stack(old_oil_type_probs_batch, axis=1)
            old_warehouse_type_probs_batch = np.stack(old_warehouse_type_probs_batch, axis=1)
            ...
            oil_type_batch = _flatten(L, N, oil_type_batch)
            warehouse_type_batch = _flatten(L, N, warehouse_type_batch)
            if self.available_actions_oil is not None:
                available_actions_oil_batch = _flatten(L, N, available_actions_oil_batch)
                available_actions_warehouse_batch = _flatten(L, N, available_actions_warehouse_batch)
            else:
                available_actions_batch = None
            ...
            old_oil_type_probs_batch= _flatten(L, N, old_oil_type_probs_batch)
            old_warehouse_type_probs_batch = _flatten(L, N, old_warehouse_type_probs_batch)
            adv_targ = _flatten(L, N, adv_targ)

            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                  value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch,\
                  adv_targ, available_actions_oil_batch,available_actions_warehouse_batch, oil_type_batch,warehouse_type_batch